...
def verify_rows(
    ...
    *,
    ...
    verified: List[Optional[BookRow]] = [None] * len(rows)
    failures = 0

    with ThreadPoolExecutor(max_workers=max(1, concurrency)) as ex:
        future_map = {
            ex.submit(_verify_one, row, timeout_s): i for i, row in enumerate(rows)
        }
    ...
    logger.info("verify: checked=%s failed=%s", len(rows), failures)
    return [r for r in verified if r is not None]
